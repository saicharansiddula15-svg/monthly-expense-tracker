...
def get_db_connection():
    # Adding an explicit timeout prevents sqlite from hanging indefinitely if a lock happens
    return sqlite3.connect("expense_tracker.db", check_same_thread=False, timeout=10)
...
def init_db():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS budget (
            budget_id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, month_str TEXT,
            amount REAL, UNIQUE(user_id, month_str), FOREIGN KEY(user_id) REFERENCES users(user_id)
        )
    """)
    conn.commit()
    conn.close()
...
def get_monthly_budget(user_id, month_str):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT amount FROM budget WHERE user_id=? AND month_str=?", (user_id, month_str))
    row = cursor.fetchone()
    conn.close()
    return float(row[0]) if row else 180000.00
        ...
        if st.button("SUBMIT SYSTEM ACCESS", type="primary", use_container_width=True):
            if username_input.strip() and password_input.strip():
                ...
                if auth_mode == "Login":
                    ...
                    conn.close() # Close IMMEDIATELY before state mutations or reruns
                    if user:
...
if not st.session_state.logged_in:
    ...
    mock_budget = get_monthly_budget(st.session_state.user_id, selected_month_str)
    ...
    with m4:
        ...
        trend_color = "#EF4444" if budget_pct > 100 else "#F59E0B"
        trend_text = "⚠️ OVER BUDGET!" if budget_pct > 100 else f"{budget_pct:.0f}% used capacity"
        
        st.markdown(f"""<div class='dashboard-metric m-budget'>
            <div class='metric-lbl'>Monthly Budget ({sel_month_tuple[0]})</div>
    ...
    if menu == "📊 Dashboard":
        ...
        # --- QUICK BUDGET ENTRY WIDGET RIGHT ON DASHBOARD ---
        with st.expander(f"⚙️ Quick Adjust Budget Cap for {selected_month_display}", expanded=False):
            with st.form("dashboard_budget_form"):
                current_budget_val = get_monthly_budget(st.session_state.user_id, selected_month_str)
                new_budget_input = st.number_input("Set Budget Maximum limit (₹)", min_value=0.0, value=current_budget_val, step=5000.0)
                if st.form_submit_button("UPDATE MONTHLY BUDGET"):
                    cursor = conn.cursor()
                    cursor.execute("""
                        INSERT INTO budget (user_id, month_str, amount) VALUES (?, ?, ?)
                        ON CONFLICT(user_id, month_str) DO UPDATE SET amount=excluded.amount
                    """, (st.session_state.user_id, selected_month_str, new_budget_input))
                    conn.commit()
                    st.success(f"Budget altered cleanly to ₹{new_budget_input:,.2f}!")
                    conn.close()
                    st.rerun()
        ...
        with b_col2:
            ...
            df_export_data = pd.read_sql_query("SELECT expense_date, category, amount, note FROM expenses WHERE user_id=?", conn, params=(st.session_state.user_id,))
            csv_buffer = df_export_data.to_csv(index=False).encode('utf-8')
            qa2.download_button(
                label="🧾 Export Spreadsheet",
                data=csv_buffer,
                file_name=f"expenses_manifest_{selected_month_str}.csv",
                mime="text/csv",
                use_container_width=True
            )
        conn.close()

    # --- VIEW 2: EXPENSE ENTRY SYSTEM ---
