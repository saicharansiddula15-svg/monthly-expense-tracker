...
if not st.session_state.logged_in:
    ...
    with m1:
        st.markdown(f"""<div class='dashboard-metric m-income'>
            ...
            <div class='metric-trend' style='color:#3B82F6;'>▲ Active <span style='color:#64748B; font-weight:normal; margin-left:4px;'>selected filter</span></div>
    ...
    if menu == "📊 Dashboard":
        ...
        with col_g1:
            ...
            # Form data for native streamlit grouping chart formats
            chart_records = []
            for m in sorted_active_months:
                ...
                chart_records.append({'Month': m, 'Income': i_sum, 'Expense': e_sum})
            
            df_chart = pd.DataFrame(chart_records)
            
            if not df_chart.empty:
                # Premium, interactive native bar chart matching your theme exactly
                st.bar_chart(
                    df_chart, 
                    x='Month', 
                    y=['Income', 'Expense'], 
        ...
        with col_g2:
            ...
            if not df_filtered_cat.empty and df_filtered_cat['amount'].sum() > 0:
                # Beautiful native horizontal breakdown chart
                st.bar_chart(df_filtered_cat, x='category', y='amount', color='#38BDF8', use_container_width=True)
            else:
