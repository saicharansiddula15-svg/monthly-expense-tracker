...
# 1. Dynamic Installation Workaround for Streamlit Cloud Environments
...
# Query totals filtered conditionally by month selection string (format: 'YYYY-MM')
...
if st.session_state.dark_theme:
    st.markdown("""
        <style>
            ...
            /* Sidebar Panel Contexts */
            section[data-testid="stSidebar"] { background-color: #0D1527 !important; border-right: 1px solid #1E293B; }
            
            /* Form & Structural Cards */
            .content-card { background-color: #111827; padding: 22px; border-radius: 14px; border: 1px solid #1E293B; margin-bottom: 20px; }
            
            /* Native Streamlit Buttons Fix for Dark Theme */
            .stButton>button { background-color: #1F2937 !important; color: #FFFFFF !important; border: 1px solid #374151 !important; border-radius: 8px !important; }
            .stButton>button:hover { background-color: #374151 !important; color: #FFFFFF !important; }
            div[data-testid="stFormSubmitButton"] button { background-color: #2563EB !important; color: #FFFFFF !important; border: none !important; }
        </style>
    ...
    st.markdown("""
        <style>
            ...
            /* Native Streamlit Buttons Fix for Light Theme */
            .stButton>button { background-color: #FFFFFF !important; color: #1F2937 !important; border: 1px solid #CBD5E1 !important; border-radius: 8px !important; }
            .stButton>button:hover { background-color: #F1F5F9 !important; color: #000000 !important; }
            div[data-testid="stFormSubmitButton"] button { background-color: #2563EB !important; color: #FFFFFF !important; border: none !important; }
        </style>
    """, unsafe_allow_html=True)
    chart_bg = '#FFFFFF'
...
if not st.session_state.logged_in:
    ...
    with t_col2:
        # Dynamic Time Matrix Selection (Year + Month Filter Combined)
        st.write(" ")
    ...
    # Fetch Data corresponding explicitly to the chosen Month string group
    ...
    if menu == "📊 Dashboard":
        ...
        with col_g1:
            ...
            # Insert dynamic log properties inside targeted array position
        ...
        with b_col1:
            ...
            if not df_rec.empty:
                ...
                st.caption("No custom transaction variables found inside database.")
    ...
    # --- VIEW 2: EXPENSE ENTRY SYSTEM ---
        ...
        with st.form("new_exp_form", clear_on_submit=True):
            ...
            amt_in = c3.number_input("Cost Amount (₹)", min_value=0.0)
            note_in = st.text_input("Operational Notes / Description Flags")
            if st.form_submit_button("ADD NEW TRANSACTION ROW"):
                if amt_in > 0:
                    cursor = conn.cursor()
                    ...
                    conn.commit()
                    st.success("Log collection updated successfully.")
                    st.rerun()
        ...
        if not df_exp.empty:
            ...
            st.data_editor(df_exp, use_container_width=True, hide_index=True, key="exp_tracker_editor")
    ...
    # --- VIEW 3: INCOME PIPELINE ---
        ...
        with st.form("inc_form", clear_on_submit=True):
            c1, c2 = st.columns(2)
            src_in = c1.text_input("Asset Inflow Stream Source Channel")
            val_in = c2.number_input("Inflow Value Base (₹)", min_value=0.0)
            if st.form_submit_button("COMMIT INCOME LOG"):
                if val_in > 0 and src_in:
                    cursor = conn.cursor()
                    cursor.execute("INSERT INTO income(user_id, income_date, source, amount, note) VALUES(?,?,?,?,?)",
                                   (st.session_state.user_id, str(datetime.today().date()), src_in, val_in, ""))
                    conn.commit()
                    st.success("Income registry metrics calculated successfully.")
                    st.rerun()
